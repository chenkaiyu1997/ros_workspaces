...
class OccupancyGrid2d(object):
    ...
    def SensorCallback(self, msg):
        ...
        for idx, r in enumerate(msg.ranges):
            ...
            scan_x = sensor_x + r*np.cos(angle)
            scan_y = sensor_y + r*np.sin(angle)
            scan_voxel = self.PointToVoxel(scan_x, scan_y)
            self._map[scan_voxel[0]][scan_voxel[1]] += self._occupied_update
            seen.add(scan_voxel)

            for d in np.arange(r, 0, -min(self._x_res, self._y_res)):
                x = sensor_x + d*np.cos(angle)
                y = sensor_y + d*np.sin(angle)
                voxel = self.PointToVoxel(x, y)
                if voxel not in seen:
                    self._map[voxel[0]][voxel[1]] += self._free_update
                    seen.add(voxel)
